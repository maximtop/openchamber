...
- Go → Back/Forward uses the shared session-history commands. Those two menu items do not register fixed native accelerators: the renderer owns their customizable keyboard bindings and prevents duplicate navigation. Other native menu accelerators keep their existing ownership.
